...
export async function getAllChallengesController(req,res){
    try {
        ...
        // }
        const completedChallenges = await challengeModel.find({user}).populate('user');

        const ongoingChallenges = await challengeModel.find({user, remainingTime:{$gt: 0}}).populate('user');
